...
If `optics.setRGBVCT` is populated, a combined value is used in the format red,green,blue,brightness,colour_temp. This is useful for lamps that support seperate RGB and CT modes. In the RGB mode, the red, green, and blue components (each ranging from 0-255) are set based on the hue and saturation published by Homekit from the _Colour_ control, while the brightness component is set based on the brighness published by Homekit; the colour_temp is always set to 0. In the CT mode, the colour temperature component is set based on the value published by Homekit from the _Temperature_ control. In this case the RGB values all get set to 255 and the brightness (ranging from 0-100) and color temperature (ranging from 140-500) are set to their appropriate values from Homekit. On/off may be set with `setOn` (otherwise "0,0,0,0,0" indicates off); brightness, hue, saturation and temperature topics are ignored.
...
```javascript
{
    ...
    "topics":
    {
        ...
        "getRGBVCT":         "<in RGBVCT mode, topic to get comma-separated red, green, blue, brightness, colour_temp>",
        "setRGBVCT":         "<in RGBVCT mode, topic to set comma-separated red, green, blue, brightness, colour_temp>",
